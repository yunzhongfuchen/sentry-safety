...
1. **主体绑定**：组内逐对象条件的左侧必须都是判定主体（UI 锁死不可选）；其余条件只能是全局类。遍历主体的每个检出框，组内全部条件对**同一个对象**成立 → 该对象为报警框。防止"张三踩架子、李四没戴安全带"被拼成误报。
2. **纯全局组**：组内全是全局条件时无需判定主体，命中报全场（无框）。
3. **组间或**：任一条件组命中即 detected=true；报警框 = 所有命中组主体框的并集。
4. **综合置信度**：组内有关系条件时，取各匹配对 min(左conf, 右conf) 的最大值；纯全局条件命中时按 1.0 计（无框命中不能被告警阈值挡死）。
5. **ROI 语义**：摄像头级 ROI 只过滤判定主体和数量条件的计数对象，关系右侧的参照物不过滤（高处区域内的人∩区域边缘的脚手架仍可判定）。
...
"welding_no_helmet": {
  ...
  "rule": {
    "groups": [{
      "primary": {"model_key": "chemical14", "classes": [1]},
      "conditions": [
...
| 文件 | 改动 |
|---|---|
| backend/detection_registry.py | 新 Schema 校验（算子合法、类别存在于模型、primary 合法）；算子列表注册并下发前端；启动迁移：老单模型字段（model_key/classes/model_confidence）→ models + rule；**删除** min_box_count/max_box_count/box_count_mode 及 yolo_box 相关读取 |
...
- 条件行：左侧类型选择器（模型▼ + 类别多选▼ + 可选conf）+ 算子下拉（关系类/数量类分组）+ 按算子类型动态渲染右侧（类型选择器 + 参数 或 数值输入）
- 条件组卡片：判定主体下拉（组内逐对象条件左侧锁死为主体）、"添加条件"（且）、"添加条件组"（或）
